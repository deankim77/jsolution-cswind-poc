...
- BOM 편집기의 REUSE/CURRENT/REFERENCE 제목은 영문 라벨·제목의 같은 높이 2줄로 맞춘다. 중복 편집 안내는 제거하고 타인 잠금 정보는 상단에서 제공한다. 편집 도구는 글자 14px·아이콘 18px·버튼 34px 공통 토큰을 사용하며 CURRENT의 구조/PART·편집 도구·수량은 한 줄로 배치한다. REFERENCE 선택기는 한 줄 높이만 사용하고 빈 도구 행을 만들지 않는다. REUSE 검색·탭은 두 줄을 유지한다.
