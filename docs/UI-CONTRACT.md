...
- 분석 요약을 AI 분석 결과로 변경하고 최신 분석/분석 이력 내부 탭을 제공한다. 분석 이력은 원본 자료별 저장된 초안 버전을 읽기 전용으로 표시하며 선택한 버전을 공통 AI 대화에서 요약·비교한다. 이력 질문은 최신 초안을 변경하지 않는다. CompRev 변경은 기존 확정 값과 비교해 선택 반영하며 빈 값으로 덮어쓰지 않고 기존 품번·BOM 구조·수량을 보존한다.
