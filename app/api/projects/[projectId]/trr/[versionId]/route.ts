import {trrPreviewCsp} from '../../../../../../services/trr-preview';
...
export async function GET(request:Request,{params}:Params){try{
 ...
 if(new URL(request.url).searchParams.get('preview')==='1')return new Response(await service.preview(s,versionId),{headers:{'Content-Type':'text/html; charset=utf-8','Cache-Control':'private, no-store','Content-Security-Policy':trrPreviewCsp,'X-Content-Type-Options':'nosniff'}});
