...
export async function POST(request:Request){
  ...
  for(const row of children)statements.push(db.prepare("INSERT INTO product_bom_items (id,company_id,parent_part_id,child_part_id,quantity,unit,sort_order,note,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?)").bind(crypto.randomUUID(),context.companyId,id,row.childPartId,row.quantity===null?null:Number(row.quantity),String(row.unit??""),Number(row.sortOrder)||0,row.note??null,t,t));
