...
export default function PartRelationsCards({partId,refreshKey,parents,children}:{partId:string;refreshKey:number;parents:number;children:number}){
 ...
 <section className="part-relation-details"><FileText size={20}/><div><b>연관 도면</b>{!data&&!error?<span>조회 중…</span>:data?.drawings.length?data.drawings.map(d=><a className="part-drawing-line" key={d.id} title={`${d.kind==='part'?'해당 부품 도면':'상위 ASSY 도면'} · ${d.drawingNumber||'도면번호 미확인'} · ${d.revision?`Rev.${d.revision}`:'Revision 미확인'} · ${d.fileName} · ${d.projectName}`} href={`/api/projects/${d.projectId}/customer-data/${d.id}?preview=1&popup=1`} target="_blank" rel="noopener noreferrer">{d.kind==='part'?'해당 부품 도면':'상위 ASSY 도면'} · {d.drawingNumber||'도면번호 미확인'} · {d.revision?`Rev.${d.revision}`:'Revision 미확인'} · {d.fileName}</a>):!error&&<span>연결된 고객 도면이 없습니다.</span>}</div></section>
 ...
 <section className="part-relation-details"><FolderOpen size={20}/><div><b>사용 프로젝트{data?` (${data.projects.length})`:''}</b>{!data&&!error?<span>조회 중…</span>:data?.projects.length?data.projects.map(p=><button type="button" className="part-project-link" key={p.id} onClick={()=>window.dispatchEvent(new CustomEvent("v2-open-project-bom",{detail:{projectId:p.id,projectName:p.name}}))}><strong>{p.name}</strong><small>{p.code}</small></button>):!error&&<span>BOM에서 사용 중인 프로젝트가 없습니다.</span>}</div></section>
 </div>;
}
