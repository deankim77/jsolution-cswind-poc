...
export default function ProductionDrawings({projectId}:{projectId:string}){
 ...
 const rows=records.flatMap(record=>{const draft=reviews.find(r=>r.recordId===record.id)?.draft;if(!draft||draft.documentType!=='drawing'||!draft.documentTypeConfirmed)return [];const title=typeof draft.drawingTitle==='string'?draft.drawingTitle.trim():'';return [{record,draft,title}]}).filter(({record,draft,title})=>`${customerReceiptId(record)} ${title} ${draft.drawingNumber} ${draft.revisionLabel}`.toLowerCase().includes(query.toLowerCase()));
