...
export default function GlobalRail({onSearch,onView,active,productionProject=false,onPlmActiveChange}:{onPlmActiveChange:(active:boolean)=>void;productionProject?:boolean;onSearch:()=>void;onView:(view:RailWorkspaceView)=>void;active:string}){
  ...
  return <><aside className="wv2-rail"><button className="wv2-brand">J</button>{primaryNav.map(renderItem)}<div className="wv2-rail-spacer"/>{personalNav.map(renderItem)}</aside>{mounted&&<><ProjectPlmTabsBridge productionProject={productionProject} onActiveChange={onPlmActiveChange}/><DocumentPreviewOverlayBridge/><BomCentralTabBridge/><BomEditorCommandBridge/></>}</>;
}
