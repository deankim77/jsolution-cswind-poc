...
import {DRAWING_AVAILABILITY_LABELS,DRAWING_AVAILABILITY,WEIGHT_SOURCES,defaultDocumentRootQuantity,validateBomFacts,type BomFact,type BomRow} from '../../lib/pbom-contract';
...
export default function PbomEditDialog({row,rows,onSave,onClose}:{row:BomRow;rows:BomRow[];onSave:(id:string,fact:BomFact)=>void|Promise<void>;onClose:()=>void}) {
 ...
 const [form,setForm]=useState<BomFact>(()=>({...defaultDocumentRootQuantity(row).bom}));
 const [saving,setSaving]=useState(false),[error,setError]=useState('');
 useEffect(()=>{const element=dialog.current;element?.showModal();return()=>element?.close()},[]);
 const change=<K extends keyof BomFact>(key:K,value:BomFact[K])=>setForm(f=>({...f,[key]:value,...(key==='partType'&&value==='ASSEMBLY'?{unit:'PCS'}:{})}));
