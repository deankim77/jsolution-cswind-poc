...
export function PartBomWorkspace({onOpenBomCompare,initialRootId,onBomCopied}:{onOpenBomCompare?:(leftRootId:string,rightRootId?:string)=>void;initialRootId?:string;onBomCopied?:(result:BomCopiedResult)=>void}){
  ...
  <section className="pbw-shell">
    ...
    {view==="bom"&&<div className="pbw-bom-screen">
      <div className="pbw-bom-command"><div><select value={activeId} disabled={isMyLock} onChange={event=>setCurrentId(event.target.value)}>{bomCandidates.map(part=><option key={part.id} value={part.id}>{part.partNumber} · {part.name} · {roleMeta(part.partType).label}</option>)}</select><span className={`mode ${isMyLock?"editing":lockedByOther?"locked":""}`}>{isMyLock?null:lockedByOther?<><LockKeyhole size={18}/>{activeLock?.lockedByName||"다른 사용자"} 편집 중</>:<><LockKeyhole size={18}/>조회 모드</>}</span></div><div><button disabled={copyBusy||deleteBusy||!activeId||!hasChildren(activeId)} onClick={()=>setCopyConfirmOpen(true)}><Copy size={18}/>{copyBusy?"복사 생성 중…":"BOM 복사 생성"}</button><button onClick={()=>setReferenceOpen(value=>!value)} className={referenceOpen?"active":""}><PanelRightOpen size={18}/>Reference BOM</button>{isMyLock?<button className="primary" disabled={lockBusy||deleteBusy} onClick={()=>void changeLock("checkin-bom")}><Check size={18}/>{lockBusy?"처리 중":"Check-in"}</button>:<button className="primary" disabled={lockBusy||deleteBusy||lockedByOther||!activeId} onClick={()=>void changeLock("checkout-bom")}><LockKeyhole size={18}/>{lockedByOther?"편집 중":lockBusy?"처리 중":"편집 시작"}</button>}<button className="danger" disabled={deleteBusy||!activeId||!hasChildren(activeId)} onClick={()=>void removeBom()}><Trash2 size={18}/>{deleteBusy?"확인 중…":"BOM 삭제"}</button></div></div>
      <div className={`pbw-editor-grid ${referenceOpen?"with-reference":""}`}>
        <aside className="pbw-reuse"><header><div><small>REUSE LIBRARY</small><b>PART / BOM 찾기</b></div><FileSearch size={18}/></header><div className="pbw-editor-tools pbw-reuse-tools"><label><Search size={18}/><input value={query} onChange={event=>setQuery(event.target.value)} placeholder="코드·품명 검색"/></label><nav><button className="active">PART</button><button>BOM</button></nav></div><div className="pbw-reuse-list">{visibleParts.slice(0,30).map(part=>{const meta=roleMeta(part.partType),displayRole=hasChildren(part.id)&&normalizedRole(part.partType)==="PART"?ITEM_ROLES[1]:meta,isRoot=part.id===activeId,isAdding=bomAddingId===part.id;return <button key={part.id} className={`${isMyLock?"editable":""} ${isRoot?"current-root":""} ${isAdding?"adding":""}`} onClick={()=>void addExistingPart(part)} title={isRoot?"현재 BOM Root":isMyLock?"현재 BOM에 추가":"편집 시작 후 추가 가능"}><span className={`pbw-part-icon ${displayRole.value.toLowerCase()}`}><Box size={18}/></span><span><b>{part.partNumber}</b><em>{part.name}</em><small>{displayRole.label}{isRoot?" · Current Root":""}</small></span>{isAdding?<span className="pbw-reuse-adding">추가 중</span>:isMyLock&&!isRoot?<Plus size={18}/>:<ChevronRight size={18}/>}</button>})}</div></aside>
        <main className="pbw-current"><header><div><small>CURRENT BOM</small><h2>{activePart?`${activePart.partNumber} · ${activePart.name}`:"BOM을 선택해 주세요"}</h2></div><strong>{activePart?`Rev.${activePart.revision}`:"—"}</strong></header><div className="pbw-tree-head pbw-editor-tools"><span>구조 / PART</span><span>수량</span></div><div className="pbw-tree-root"><Layers3 size={18}/><span><b>{activePart?.partNumber||"—"}</b><em>{activePart?.name||"BOM 없음"}</em><small>{activePart?roleMeta(activePart.partType).label:""}</small></span></div><div className="pbw-tree-body">{activeId&&renderTree(activeId)}{activeId&&!hasChildren(activeId)&&<p className="pbw-empty">현재 PART를 Root로 열었습니다. 아직 하위 구조가 없습니다.</p>}</div></main>
        {referenceOpen&&<aside className="pbw-reference"><header><div><small>REFERENCE BOM · READ ONLY</small><b>다른 BOM에서 가져오기</b></div><button onClick={()=>setReferenceOpen(false)} aria-label="참조 BOM 닫기"><X size={18}/></button></header><div className="pbw-editor-tools pbw-reference-tools"><select value={activeReferenceId} onChange={event=>setReferenceId(event.target.value)}><option value="">참조 BOM 선택</option>{referenceCandidates.map(part=><option key={part.id} value={part.id}>{part.partNumber} · {part.name}</option>)}</select></div><div className="pbw-reference-root"><Layers3 size={18}/><span><b>{referencePart?.partNumber||"—"}</b><em>{referencePart?.name||"참조 BOM을 선택해 주세요"}</em></span></div><div className="pbw-reference-tree">{activeReferenceId&&renderTree(activeReferenceId,true)}</div><footer>Assembly / Part Drag & Drop은 편집 기능 단계에서 연결합니다.</footer></aside>}
      </div>
    </div>}

    {partCreateOpen&&<div className="pbw-create-backdrop" onMouseDown={event=>{if(event.target===event.currentTarget&&!partSaving)setPartCreateOpen(false)}}><aside className="pbw-create-panel"><header><div><small>PART MASTER</small><h2>PART 생성</h2><p>품번은 Identity로 관리하고 Item Role과 부품 유형은 참조 속성으로 지정합니다.</p></div><button onClick={()=>!partSaving&&setPartCreateOpen(false)} aria-label="닫기"><X size={18}/></button></header><div className="pbw-create-body"><section><label className="pbw-create-label">Item Role</label><div className="pbw-role-options">{ITEM_ROLES.map(role=><button type="button" key={role.value} className={partForm.itemRole===role.value?"active":""} onClick={()=>setPartForm(current=>({...current,itemRole:role.value}))}><b>{role.label}</b><span>{role.hint}</span></button>)}</div><small className="pbw-create-help">Item Role은 품번과 분리됩니다. 역할이 바뀌어도 PART Code는 유지됩니다.</small></section><section><label className="pbw-create-label">PART Code</label><div className="pbw-number-mode"><button type="button" className={partForm.numberMode==="auto"?"active":""} onClick={()=>setPartForm(current=>({...current,numberMode:"auto",partNumber:""}))}>자동채번</button><button type="button" disabled={!allowManual} className={partForm.numberMode==="manual"?"active":""} onClick={()=>allowManual&&setPartForm(current=>({...current,numberMode:"manual"}))}>직접입력</button></div>{partForm.numberMode==="manual"?<input value={partForm.partNumber} onChange={event=>setPartForm(current=>({...current,partNumber:event.target.value}))} placeholder="기존 PART Code 입력"/>:<div className="pbw-auto-number"><b>{autoNumberPreview}</b><span>Role · 부품 유형 · BOM Level · Revision과 무관한 공통 순차번호</span></div>}</section><label><span>품명 *</span><input autoFocus value={partForm.name} onChange={event=>setPartForm(current=>({...current,name:event.target.value}))} placeholder="품명을 입력해 주세요"/></label><label><span>부품 유형</span><select className="pbw-create-select" value={partForm.categoryId} onChange={event=>setPartForm(current=>({...current,categoryId:event.target.value}))}><option value="">선택 안 함</option>{partCategories.map(category=><option key={category.id} value={category.id}>{category.name}</option>)}</select><small className="pbw-create-help">회사 기준정보에서 관리되는 참조 속성입니다. BOM 구조와 Item Role에는 영향을 주지 않습니다.</small></label><label><span>사양</span><input value={partForm.spec} onChange={event=>setPartForm(current=>({...current,spec:event.target.value}))} placeholder="규격 · 재질 · 사양"/></label><label className="short"><span>단위</span><input value={partForm.unit} onChange={event=>setPartForm(current=>({...current,unit:event.target.value}))} placeholder="EA"/></label></div><footer><button onClick={()=>setPartCreateOpen(false)} disabled={partSaving}>취소</button><button className="primary" onClick={()=>void createPart()} disabled={partSaving||!partForm.name.trim()||(partForm.numberMode==="manual"&&!partForm.partNumber.trim())}>{partSaving?"생성 중…":"PART 생성"}</button></footer></aside></div>}
