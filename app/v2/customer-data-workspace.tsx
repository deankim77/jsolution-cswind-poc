...
export default function CustomerDataWorkspace({project,embedded=false}: {project:V2Project|null;embedded?:boolean;onOpenFilter?:(config:WorkspaceFilterConfig)=>void}){
 ...
 const [sourcePurpose,setSourcePurpose]=useState<CustomerSourcePurpose>('input');
 ...
 const rows=(data?.records??[]).filter(r=>`${customerReceiptId(r)} ${r.fileName} ${r.createdByName||''}`.toLowerCase().includes(query.toLowerCase())).sort((a,b)=>b.createdAt-a.createdAt||b.id.localeCompare(a.id));
 const body=<><div className="wv2-toolbar">
 {(selected||open)?<button disabled={uploading} onClick={()=>{setSelected(null);setOpen(false)}}><ArrowLeft size={18}/>목록으로</button>:<><label><Search size={18}/><input aria-label="원본 검색" value={query} onChange={e=>setQuery(e.target.value)} placeholder="자료 ID·파일명·담당자 검색"/></label><button onClick={()=>setReload(v=>v+1)}><RefreshCw size={18}/>새로고침</button></>}
 ...
 {selected?<CustomerSourcePreview projectId={project.id} record={selected}/>:open?<div className="customer-upload-area"><div className="review-decision"><label>자료 용도 <select disabled={uploading} value={sourcePurpose} onChange={e=>setSourcePurpose(e.target.value as CustomerSourcePurpose)}>{Object.entries(CUSTOMER_SOURCE_PURPOSES).map(([v,l])=><option key={v} value={v}>{l}</option>)}</select></label></div><label className={`customer-drop ${drag?'drag':''}`} onDragOver={e=>{e.preventDefault();setDrag(true)}} onDragLeave={()=>setDrag(false)} onDrop={e=>{e.preventDefault();setDrag(false);void upload(Array.from(e.dataTransfer.files))}}><input className="customer-upload-picker" aria-label="고객 원본 파일 선택" ref={input} multiple type="file" disabled={uploading} onChange={e=>void upload(Array.from(e.target.files??[]))}/><Upload size={24}/><strong>{uploading?'고객 자료를 접수하고 있습니다…':'여러 파일을 여기에 놓거나 클릭해서 선택하세요'}</strong><span>파일당 최대 50MB · 동일 내용은 중복 제외 · 문서 분류는 AI Data Review에서 확인</span></label><p>{uploading?'업로드 완료까지 이 화면을 유지하세요.':'파일을 선택하면 바로 업로드합니다. 실패한 파일만 다시 선택할 수 있습니다.'}</p><table className="production-table"><thead><tr><th>원본 파일명</th><th>처리 결과</th></tr></thead><tbody>{results.map((r,i)=><tr key={i}><td>{r.name}</td><td role="status">{r.status}</td></tr>)}</tbody></table></div>:<div className="cswind-data-table"><table><thead><tr><th>자료 ID</th><th>원본 파일명 / 미리보기</th><th>자료 용도</th><th>업로드 담당자</th><th>업로드 일시</th><th>다운로드</th></tr></thead><tbody>{rows.map(r=><tr key={r.id}><td><CustomerSourceLink record={r}/></td><td><button onClick={()=>setSelected(r)}>{r.fileName}</button></td><td>{CUSTOMER_SOURCE_PURPOSES[r.sourcePurpose??'input']}</td><td>{r.createdByName||r.createdBy}</td><td>{uploadTime(r.createdAt)}</td><td><a aria-label={`${r.fileName} 다운로드`} href={`/api/projects/${project.id}/customer-data/${r.id}`}><Download size={18}/></a></td></tr>)}{!rows.length&&<tr><td colSpan={6}>{!data&&!error?'고객 자료를 불러오는 중…':error?'조회에 실패했습니다. 새로고침하세요.':'접수된 자료가 없습니다. + 원본 등록으로 파일을 올려주세요.'}</td></tr>}</tbody></table></div>}
 </section></>;
