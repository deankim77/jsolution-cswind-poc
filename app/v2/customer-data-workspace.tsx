...
export default function CustomerDataWorkspace({project,embedded=false}: {project:V2Project|null;embedded?:boolean;onOpenFilter?:(config:WorkspaceFilterConfig)=>void}){
 ...
 async function upload(files:File[],sourcePurpose:CustomerSourcePurpose){if(!project||running.current||!data?.canUpload||!files.length)return;running.current=true;setUploading(true);setResults(files.map(f=>({name:f.name,status:'대기',purpose:sourcePurpose})));setError('');
 ...
 <span className="cswind-data-count">{open?'고객 자료 일괄 업로드':`${rows.length}개 원본 · 최신 업로드순`}</span>
