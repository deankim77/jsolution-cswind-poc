...
import CustomerDataDeleteButton from './customer-data-delete-button';
...
export function CustomerThumbnail({projectId,record:r}:{projectId:string;record:CustomerDataRecord}){return <button type="button" className="customer-thumb" aria-label={`${r.fileName} 원본 미리보기`} title={`${r.fileName} 팝업으로 보기`} onClick={()=>{window.open(`/api/projects/${projectId}/customer-data/${r.id}?preview=1&popup=1`,'_blank',`popup=yes,width=${window.screen.availWidth},height=${window.screen.availHeight},left=0,top=0,resizable=yes,scrollbars=yes,noopener,noreferrer`)}}>{/\.(png|jpe?g|webp)$/i.test(r.fileName)?<img loading="lazy" alt="원본 미리보기" src={`/api/projects/${projectId}/customer-data/${r.id}?preview=1`}/>:<FileText size={18}/>}</button>;}
...
export default function CustomerDataWorkspace({project,embedded=false,purposeScope}: {project:V2Project|null;embedded?:boolean;purposeScope?:'ttr';onOpenFilter?:(config:WorkspaceFilterConfig)=>void}){
 ...
 const [columns,setColumns]=useState({purpose:'',owner:''});
 useEffect(()=>{setColumns({purpose:'',owner:''});setSelected([])},[project?.id]);
 const [detailId,setDetailId]=useState('');
 const [selected,setSelected]=useState<string[]>([]);
 const running=useRef(false),mounted=useRef(true);
 useEffect(()=>{mounted.current=true;return()=>{mounted.current=false}},[]);
 useEffect(()=>{const refresh=()=>{if(!running.current)setReload(value=>value+1)};window.addEventListener('focus',refresh);window.addEventListener('v2-deliverables-updated',refresh);return()=>{window.removeEventListener('focus',refresh);window.removeEventListener('v2-deliverables-updated',refresh)}},[]);
 ...
 running.current=false;if(mounted.current){setUploading(false);setReload(x=>x+1);window.dispatchEvent(new CustomEvent("v2-deliverables-updated",{detail:{projectId:project.id}}));}}
 if(!project)return <p>생산 프로젝트를 선택하세요.</p>;
 const sourceRows=(data?.records??[]).filter(r=>(!purposeScope||r.sourcePurpose===purposeScope)&&`${customerReceiptId(r)} ${r.fileName} ${r.createdByName||''}`.toLowerCase().includes(query.toLowerCase())).sort((a,b)=>b.createdAt-a.createdAt||b.id.localeCompare(a.id));
 const purposeValue=(r:CustomerDataRecord)=>CUSTOMER_SOURCE_PURPOSES[r.sourcePurpose??'input'];
 const ownerValue=(r:CustomerDataRecord)=>r.createdByName||r.createdBy;
 const rows=sourceRows.filter(r=>(!columns.purpose||JSON.stringify(purposeValue(r).trim())===columns.purpose)&&(!columns.owner||JSON.stringify(ownerValue(r).trim())===columns.owner));
 const uploadPurposes=['bom','ttr','supplied'] as const;
 ...
 {!uploadOpen&&<CustomerDataDeleteButton projectId={project.id} ids={selected} disabled={!data?.canUpload||uploading} onDeleted={()=>{setSelected([]);setData(null);setReload(v=>v+1)}}/>}
 ...
 {uploadOpen?<div className="customer-upload-area"><p className="customer-upload-guidance">BOM 도면, TRR 기술문서 또는 사급품 Excel 원본을 용도별로 등록하세요.</p><div className="customer-upload-cards">{uploadPurposes.map(purpose=><label key={purpose} className={`customer-drop ${drag===purpose?'drag':''}`} onDragOver={e=>{e.preventDefault();if(!uploading)setDrag(purpose)}} onDragLeave={()=>setDrag(null)} onDrop={e=>{e.preventDefault();setDrag(null);void upload(Array.from(e.dataTransfer.files),purpose)}}><input className="customer-upload-picker" aria-label={`${CUSTOMER_SOURCE_PURPOSES[purpose]} 파일 선택`} multiple type="file" accept={purpose==='supplied'?'.xlsx':undefined} disabled={uploading} onChange={e=>{const files=Array.from(e.target.files??[]);e.target.value='';void upload(files,purpose)}}/><Upload size={24}/><strong>{CUSTOMER_SOURCE_PURPOSES[purpose]}</strong><span>{uploading?'고객 자료를 접수하고 있습니다…':'파일을 끌어다 놓거나 클릭해서 선택하세요'}</span><span>여러 파일 선택 · 파일당 최대 50MB</span></label>)}</div><table className="production-table"><thead><tr><th>원본 파일명</th><th>자료 용도</th><th>처리 결과</th></tr></thead><tbody>{results.map((r,i)=><tr key={i}><td>{r.name}</td><td>{CUSTOMER_SOURCE_PURPOSES[r.purpose]}</td><td role="status">{r.status}</td></tr>)}</tbody></table></div>:<div className="cswind-data-table"><table><thead><tr><th><input type="checkbox" aria-label="원본 전체 선택" checked={rows.length>0&&rows.every(r=>selected.includes(r.id))} onChange={e=>setSelected(e.target.checked?rows.map(r=>r.id):[])}/></th><th>미리보기</th><th>자료 ID</th><th>원본 파일명</th><th>자료 용도<ColumnValueFilter label="자료 용도" values={sourceRows.map(purposeValue)} value={columns.purpose} onChange={purpose=>{setColumns(c=>({...c,purpose}));setSelected([])}}/></th><th>업로드 담당자<ColumnValueFilter label="담당자" values={sourceRows.map(ownerValue)} value={columns.owner} onChange={owner=>{setColumns(c=>({...c,owner}));setSelected([])}}/></th><th>업로드 일시</th><th>다운로드</th></tr></thead><tbody>{rows.map(r=><tr key={r.id}><td><input type="checkbox" aria-label={`${r.fileName} 선택`} checked={selected.includes(r.id)} onChange={e=>setSelected(ids=>e.target.checked?[...ids,r.id]:ids.filter(id=>id!==r.id))}/></td><td><CustomerThumbnail projectId={project.id} record={r}/></td><td><button onClick={()=>setDetailId(r.deliverableId||'')}>{customerReceiptId(r)}</button></td><td title={r.fileName}><button onClick={()=>setDetailId(r.deliverableId||'')}>{r.fileName}</button></td><td>{CUSTOMER_SOURCE_PURPOSES[r.sourcePurpose??'input']}</td><td>{r.createdByName||r.createdBy}</td><td>{uploadTime(r.createdAt)}</td><td><a aria-label={`${r.fileName} 다운로드`} href={`/api/projects/${project.id}/customer-data/${r.id}`}><Download size={18}/></a></td></tr>)}{!rows.length&&<tr><td colSpan={8}>{!data&&!error?'고객 자료를 불러오는 중…':error?'조회에 실패했습니다. 새로고침하세요.':purposeScope?'등록된 TRR 자료가 없습니다. 고객 Data에서 등록해 주세요.':'접수된 자료가 없습니다. + 원본 등록으로 파일을 올려주세요.'}</td></tr>}</tbody></table></div>}
 </section>{detailId&&<DocumentManagementPanel deliverableId={detailId} onClose={()=>setDetailId('')} onOpenTask={(projectId,taskId)=>{setDetailId('');window.dispatchEvent(new CustomEvent('v2-open-wbs-task',{detail:{projectId,taskId}}))}}/>}</>;
