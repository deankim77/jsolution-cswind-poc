...
function DeliverableList({rows,versions,taskMap,onOpen}:{rows:Deliverable[];versions:Version[];taskMap:Map<string,Task>;onOpen:(item:Deliverable)=>void}){
  const [filters,setFilters]=useState({kind:'',registration:'',status:''});
  const registration=(item:Deliverable)=>registered(item,versions)?'등록':'미등록';
  const visible=rows.filter(item=>(!filters.kind||JSON.stringify(sourceLabel(item))===filters.kind)&&(!filters.registration||JSON.stringify(registration(item))===filters.registration)&&(!filters.status||JSON.stringify(statusLabel(item.status))===filters.status));
