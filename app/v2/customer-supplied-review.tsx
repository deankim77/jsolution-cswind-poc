...
import {suggestedSuppliedProcess,type SuppliedProcess,suppliedDecision,type SuppliedData,type SuppliedFact,type SuppliedChoice} from '../../lib/customer-supplied-contract';
...
export default function CustomerSuppliedReview({projectId,recordId,version,facts,area,disabled,onApplied,documentName,actionContainer,renderSource}:{projectId:string;recordId:string;version:number;facts:SuppliedFact[];area:'summary'|'supplied'|'bom';disabled:boolean;onApplied:()=>void;documentName:string;actionContainer?:HTMLElement|null;renderSource?:(ids:string[])=>React.ReactNode}){
 const [processing,setProcessing]=useState<Record<string,SuppliedProcess>>({});
 ...
 {area==='bom'?<>{pbom&&<PbomTable variant="supplied" rows={reviewRows} renderSource={renderSource} renderCell={(key,row,fallback)=>row.partId?fallback:key==='item'?row.bom.customerItemNumber:key==='description'?row.bom.itemDescription:key==='status'?'미연결':'—'} extraColumn={{label:'사급수량',render:quantity}}/>}</>:<CustomerSuppliedTable rows={facts.map(f=>({...f,changeLabel:data?suppliedDecision(f,data.entries):'조회 중'}))} processing={decisions} onProcessing={area==='supplied'?(id,value)=>setProcessing(current=>({...current,[id]:value})):undefined} selected={selected} onSelect={area==='summary'?undefined:setSelected} disabled={locked}/>}
 </section>;
}
