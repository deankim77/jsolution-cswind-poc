...
type WorkspaceView="customer-data"|"portfolio"|"projects"|"dashboard"|"edit-project"|"create-project"|"templates"|"template-editor"|"workflow-templates"|"wbs"|"documents"|"product-data"|"bom-compare"|"cost"|"workflows"|"create-workflow"|"ecr"|"create-ecr"|"quality"|"create-quality"|"issues"|"reports"|"my-work"|"my-ai"|"settings"|"artifact-preview";
const workspaceLabels:Record<WorkspaceView,string>={"customer-data":"고객 Data",portfolio:"전사 대시보드",projects:"프로젝트 목록",dashboard:"프로젝트 대시보드","edit-project":"프로젝트 정보 수정","create-project":"새 프로젝트",templates:"프로젝트 템플릿","template-editor":"템플릿 편집","workflow-templates":"워크플로우 템플릿",wbs:"일정 · WBS",documents:"문서 · 산출물","product-data":"PART · BOM","bom-compare":"BOM 비교",cost:"원가 관리",workflows:"워크플로우","create-workflow":"새 워크플로우",ecr:"설계변경","create-ecr":"새 설계변경",quality:"품질관리","create-quality":"새 품질문제",issues:"이슈 · 리스크",reports:"보고 · 회의","my-work":"MY WORK","my-ai":"MY AI HOME",settings:"시스템 설정","artifact-preview":"AI 결과 미리보기"};
...
export default function NewWorkViewApp(){
  ...
  const activateWorkspaceTab=(tab:WorkspaceTab)=>{setActiveWorkspaceTab(tab.key);setWorkspaceViewState(tab.view);if(tab.view==="template-editor")setTemplateEditorId(tab.entityId);if(tab.view==="customer-data")setPanelOpen(false);if((tab.view==="wbs"||tab.view==="customer-data")&&tab.entityId){const next=projects.find(item=>item.id===tab.entityId);if(next)setProject(next)}};
  ...
  useEffect(()=>{const active=workspaceTabs.find(item=>item.key===activeWorkspaceTab);if((active?.view!=="wbs"&&active?.view!=="customer-data")||!active.entityId||!projects.length)return;const next=projects.find(item=>item.id===active.entityId);if(next&&next.id!==project?.id)setProject(next)},[projects,activeWorkspaceTab]);
  ...
  return <main className={`wv2 ${contextOpen?"context-open":""} ${panelOpen?"panel-open":""}`}>
    <GlobalRail onSearch={()=>setGlobalOpen(true)} onView={setWorkspaceView} active={workspaceView} />
     {contextOpen&&<ContextNav project={project} projects={projects} user={sessionUser} active={workspaceView} onView={setWorkspaceView} onProject={next=>{setProject(next);setSelectedId("");setPanelOpen(false);if(workspaceView==="customer-data"){if(next.projectTypeCode===PRODUCTION_PROJECT_CODE)openWorkspaceTab({key:`customer-data:${next.id}`,view:"customer-data",entityId:next.id,title:`고객 Data · ${next.name}`});else setWorkspaceView("dashboard");}if(workspaceView==="wbs")openWorkspaceTab({key:`wbs:${next.id}`,view:"wbs",entityId:next.id,title:`WBS · ${next.name}`})}} onClose={()=>setContextOpen(false)}/>}
    <section className="wv2-workspace">
      ...
      <section className="wv2-content">
        ...
        {workspaceView==="customer-data"&&<CustomerDataWorkspace key={activeWorkspace?.entityId||project?.id} project={activeWorkspace?.entityId?projects.find(item=>item.id===activeWorkspace.entityId)??null:project} onOpenFilter={openWorkspaceFilter}/>}
        ...
        {workspaceView==="projects"&&<ProjectListWorkspace projects={projects} loading={loading} onCreate={()=>setWorkspaceView("create-project")} onOpen={next=>{setProject(next);setSelectedId("");setPanelOpen(false);openWorkspaceTab({key:`wbs:${next.id}`,view:"wbs",entityId:next.id,title:`WBS · ${next.name}`})}} onOpenFilter={openWorkspaceFilter}/>}
