...
const workspaceLabels:Record<WorkspaceView,string>={portfolio:"글로벌 대시보드",projects:"프로젝트 목록",dashboard:"프로젝트 대시보드","edit-project":"프로젝트 정보 수정","create-project":"새 프로젝트",templates:"프로젝트 템플릿","template-editor":"템플릿 편집","workflow-templates":"워크플로우 템플릿",wbs:"PROJECT HUB",documents:"문서 · 산출물","product-data":"PART · BOM","bom-compare":"BOM 비교",cost:"프로젝트 원가",workflows:"워크플로우","create-workflow":"새 워크플로우",ecr:"설계변경","create-ecr":"새 설계변경",quality:"품질관리","create-quality":"새 품질문제",issues:"이슈 · 리스크",reports:"보고 · 회의","my-work":"MY WORK","my-ai":"MY AI HOME",settings:"시스템 설정","artifact-preview":"AI 결과 미리보기"};
...
export default function NewWorkViewApp(){
  ...
  const setWorkspaceView=(next:WorkspaceView)=>{if(next==="issues")setIssueProjectPreset("");if(next==="wbs"&&project){openWorkspaceTab({key:`wbs:${project.id}`,view:"wbs",entityId:project.id,title:`PROJECT HUB · ${project.name}`});return}openWorkspaceTab(genericWorkspaceTab(next))};
  ...
  useEffect(()=>{if(workspaceView!=="wbs"||!project)return;const active=workspaceTabs.find(item=>item.key===activeWorkspaceTab);if(active?.view==="wbs"&&active.entityId&&active.entityId!==project.id)return;const key=`wbs:${project.id}`;if(activeWorkspaceTab===key)return;const tab={key,view:"wbs" as const,entityId:project.id,title:`PROJECT HUB · ${project.name}`};setWorkspaceTabs(current=>{const withoutPlaceholder=current.filter(item=>item.key!=="wbs");return withoutPlaceholder.some(item=>item.key===key)?withoutPlaceholder:[...withoutPlaceholder,tab]});setActiveWorkspaceTab(key)},[workspaceView,project?.id]);
  ...
  const openWorkspaceTask=(projectId:string,taskId:string,tab:PanelTab="actual",keepWorkspace=false)=>{const next=projects.find(item=>item.id===projectId);if(!next||!taskId)return;setView("list");if(!keepWorkspace)openWorkspaceTab({key:`wbs:${next.id}`,view:"wbs",entityId:next.id,title:`PROJECT HUB · ${next.name}`});if(next.id===project?.id&&tasks.some(item=>item.id===taskId)){setSelectedId(taskId);setPanelTab(tab);setPanelOpen(true);setPendingTask(null);return}setPendingTask({projectId,taskId,tab});setProject(next)};
  useEffect(()=>{const open=(event:Event)=>{const detail=(event as CustomEvent<{projectId:string;taskId:string}>).detail;if(detail){setProductionOpen(false);openWorkspaceTask(detail.projectId,detail.taskId,"deliverable")}};window.addEventListener("v2-open-wbs-task",open);return()=>window.removeEventListener("v2-open-wbs-task",open)},[projects,project?.id,tasks]);
  ...
  return <main className={`wv2 ${contextOpen?"context-open":""} ${panelOpen?"panel-open":""}`}>
    <GlobalRail onSearch={()=>setGlobalOpen(true)} onView={setWorkspaceView} active={workspaceView} onPlmActiveChange={setPlmTabActive} productionProject={project?.projectTypeCode===PRODUCTION_PROJECT_CODE} />
     {contextOpen&&<ContextNav project={project} projects={projects} user={sessionUser} active={workspaceView} onView={setWorkspaceView} onProject={next=>{setProject(next);setSelectedId("");setPanelOpen(false);if(workspaceView==="wbs")openWorkspaceTab({key:`wbs:${next.id}`,view:"wbs",entityId:next.id,title:`PROJECT HUB · ${next.name}`})}} onClose={()=>setContextOpen(false)}/>}
    <section className="wv2-workspace">
      ...
      <section className="wv2-content">
        ...
        {workspaceView==="projects"&&<ProjectListWorkspace projects={projects} loading={loading} onCreate={()=>setWorkspaceView("create-project")} onOpen={next=>{setProject(next);setSelectedId("");setPanelOpen(false);openWorkspaceTab({key:`wbs:${next.id}`,view:"wbs",entityId:next.id,title:`PROJECT HUB · ${next.name}`})}} onOpenFilter={openWorkspaceFilter}/>}
        {workspaceView==="dashboard"&&<GateProjectDashboard onOpenAiTab={tab=>{closeCustomerAi();setProductionTab(tab);setProductionOpen(true);setProductionRefresh(n=>n+1);setWorkspaceView("wbs")}} project={project} tasks={tasks} onOpenTask={(taskId,tab="detail")=>openWorkspaceTask(project?.id||"",taskId,tab)} onNavigate={destination=>{if(destination==="gantt"){setView("gantt");setWorkspaceView("wbs");return}if(destination==="issues"){setIssueProjectPreset(project?.id||"");openWorkspaceTab(genericWorkspaceTab("issues"));return}setWorkspaceView(destination)}} onEdit={()=>setWorkspaceView("edit-project")} onAi={request=>{setAiDraft({key:Date.now(),...request});setPanelTab("ai");setPanelOpen(true)}}/>}
...
function ContextNav({project,projects,user,active,onView,onProject,onClose}:{project:Project|null;projects:Project[];user:SessionUser|null;active:WorkspaceView;onView:(view:WorkspaceView)=>void;onProject:(project:Project)=>void;onClose:()=>void}){
  ...
  return <aside className="wv2-context">
    ...
    <nav>
      ...
      {groups.project&&<div className="wv2-context-level2"><button className={active==="portfolio"?"active":""} onClick={()=>onView("portfolio")}><LayoutDashboard size={18}/>글로벌 대시보드</button><button className={active==="dashboard"?"active":""} onClick={()=>onView("dashboard")}><Target size={18}/>프로젝트 대시보드</button><button className={active==="templates"?"active":""} onClick={()=>onView("templates")}><FileStack size={18}/>프로젝트 템플릿</button><button className={active==="workflow-templates"?"active":""} onClick={()=>onView("workflow-templates")}><Workflow size={18}/>워크플로우 템플릿</button><button className={active==="create-project"?"active":""} onClick={()=>onView("create-project")}><Plus size={18}/>새 프로젝트</button><button className={active==="projects"?"active":""} onClick={()=>onView("projects")}><Folders size={18}/>프로젝트 목록</button><button className={active==="wbs"?"active":""} onClick={()=>onView("wbs")}><Workflow size={18}/>PROJECT HUB</button><button className={active==="documents"?"active":""} onClick={()=>onView("documents")}><FileText size={18}/>문서 · 산출물</button><button className={active==="product-data"?"active":""} onClick={()=>onView("product-data")}><Table2 size={18}/>PART · BOM</button><button className={active==="cost"?"active":""} onClick={()=>onView("cost")}><CircleDollarSign size={18}/>프로젝트 원가</button><button className={active==="issues"?"active":""} onClick={()=>onView("issues")}><AlertTriangle size={18}/>이슈 · 리스크</button><button className={active==="workflows"?"active":""} onClick={()=>onView("workflows")}><GitPullRequestArrow size={18}/>워크플로우</button><button className={active==="ecr"?"active":""} onClick={()=>onView("ecr")}><RefreshCw size={18}/>설계변경</button><button className={active==="quality"?"active":""} onClick={()=>onView("quality")}><ShieldCheck size={18}/>품질관리</button><button className={active==="reports"?"active":""} onClick={()=>onView("reports")}><MessageSquareText size={18}/>보고 · 회의</button></div>}
