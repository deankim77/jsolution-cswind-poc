...
import "./part-library-workspace.css";

type Part={createdAt?:number;id:string;partNumber:string;name:string;partType:string;spec?:string;unit:string;revision:string;status:string;standardCost:number;rollupCost:number};
...
export function PartLibraryWorkspace({onOpenBomCompare}:{onOpenBomCompare?:(leftRootId:string,rightRootId?:string)=>void}){
  ...
  const [query,setQuery]=useState(""),[roleFilter,setRoleFilter]=useState<ItemRole|"">("");
  ...
  const visibleParts=useMemo(()=>{const key=query.trim().toLowerCase().replace(/\s+/g,"");return data.parts.filter(item=>(!roleFilter||normalizedRole(item.partType)===roleFilter)&&(!key||`${item.partNumber}${item.name}${item.spec||""}${item.partType}${roleMeta(item.partType).label}`.toLowerCase().replace(/\s+/g,"").includes(key))).sort((a,b)=>Number(b.createdAt??0)-Number(a.createdAt??0)||b.partNumber.localeCompare(a.partNumber,undefined,{numeric:true}))},[data.parts,query,roleFilter]);
