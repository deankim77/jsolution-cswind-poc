...
export default function BomEditorCommandBridge(){
  ...
  const toolbar=<div className="pbw-command-floating"><span className="pbw-command-selection"><b>{selectedRoot?"TOP":selected?"선택":"선택 없음"}</b><em>{label(selectedPart)}</em></span><button title="위로" disabled={!editable||!selected||siblingIndex<=0||busy} onClick={moveUp}><ArrowUp size={18}/></button><button title="아래로" disabled={!editable||!selected||siblingIndex<0||siblingIndex>=siblings.length-1||busy} onClick={moveDown}><ArrowDown size={18}/></button><button title="내어쓰기" disabled={!editable||!selected||selected.row.parentPartId===rootId||busy} onClick={outdent}><IndentDecrease size={18}/></button><button title="들여쓰기" disabled={!editable||!selected||siblingIndex<=0||busy} onClick={indent}><IndentIncrease size={18}/></button><button title="전체 접기" onClick={collapseAll}><ChevronsDownUp size={18}/></button><button title="전체 펼치기" onClick={expandAll}><ChevronsUpDown size={18}/></button><button className="new" title="신규 PART 생성" disabled={!editable||(!selected&&!selectedRoot)||busy} onClick={openNewPart}><Plus size={18}/>신규 PART</button><button className="danger" title="연결 해제" disabled={!editable||!selected||busy} onClick={disconnect}><Unlink size={18}/></button></div>;
