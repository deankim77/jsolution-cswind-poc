...
export default function CustomerSuppliedWorkspace({projectId}:{projectId:string}){
 ...
 return <><div className="wv2-toolbar"><label><Search size={18}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="SECTION·고객 품목번호·품명 검색"/></label><button onClick={()=>setReload(n=>n+1)}><RefreshCw size={18}/>새로고침</button><label className="supplied-include-removed"><input type="checkbox" checked={removed} onChange={e=>setRemoved(e.target.checked)}/>삭제·대체 이전 품목 포함</label><span className="cswind-data-count">{error?'조회 실패':data?`${rows.length}개 사급품`:'조회 중…'}</span></div><section className="wv2-canvas">{error&&<p role="alert" className="wv2-form-error">{error}</p>}{error?null:!data?<p>사급품 목록을 불러오는 중…</p>:<CustomerSuppliedTable rows={rows.map(r=>({...r,changeLabel:r.status==='removed'?'삭제':SUPPLIED_CHANGE_LABELS[r.change as keyof typeof SUPPLIED_CHANGE_LABELS]??r.change,source:`원본 ${r.recordId} · 분석 v${r.version} · ${r.bomApplied?'BOM 반영 완료':'BOM 미반영'}`}))}/>}</section></>;
}
