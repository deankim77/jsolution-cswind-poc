...
import {suppliedMissingData,suppliedAnalysisSummary} from '../../lib/customer-supplied-contract';
...
type Props={url:string;recordId:string;version:number;projectId:string;projectName:string;fileName:string;busy:boolean;canReview:boolean;hasConfirmed:boolean;onBusy:(value:boolean)=>void;onApplied:()=>void;onArea?:(area:ReviewArea|'supplied')=>void;children?:ReactNode};
type ReviewListResponse={reviews:ReviewState[]};
const areaNames:Record<ReviewArea|'supplied',string>={supplied:'사급품',pbom:'BOM',extract:'AI 추출사항',trr:'TRR'};
function legacyMissingData(draft:ReviewDraft):MissingReviewData[]{
 if(draft.documentType==='supplied')return suppliedMissingData(draft.suppliedItems??[]);
...
function analysisSummary(draft:ReviewDraft){return draft.documentType==='supplied'?suppliedAnalysisSummary(draft.suppliedItems??[]):draft.summary;}
function analysisAreas(draft:ReviewDraft):(ReviewArea|'supplied')[]{return draft.documentType==='supplied'?['supplied','pbom']:Object.keys(REVIEW_AREAS) as ReviewArea[];}
function areaItems(draft:ReviewDraft,area:ReviewArea|'supplied'){
 if(draft.documentType==='supplied'&&(area==='supplied'||area==='pbom'))return (draft.suppliedItems??[]).map(f=>({id:f.id,title:`${f.itemNumber} · ${f.description}`,detail:f.changeText,area,source:f.source,trrSection:undefined}));
 return draft.items.filter(item=>item.area===area);
}
...
export default function CustomerAnalysisResults({url,recordId,version,projectId,projectName,fileName,busy,canReview,onBusy,onApplied,onArea}:Props){
 ...
 return <section className="customer-analysis-results">
  ...
  {tab==='latest'?loading?<p className="analysis-empty">최신 분석 결과를 불러오는 중…</p>:!draft?<p className="analysis-empty">AI 분석을 실행하면 최신 분석 결과를 확인할 수 있습니다.</p>:draft.analysisMode==='text'&&draft.documentType!=='supplied'?<section className="analysis-card"><h2>AI 분석 결과</h2><p className="analysis-history-summary">{analysisSummary(draft)}</p></section>:<div className="analysis-latest">
   ...
   <section className="analysis-card">
    ...
    {draft.items.some(i=>i.area==='trr')&&<p className="analysis-history-summary">{analysisSummary(draft)}</p>}
   ...
   <section className="analysis-card">
    ...
    <div className="analysis-data-counts">{analysisAreas(draft).map(area=><div key={area}><strong>{areaItems(draft,area).length}</strong><span>{draft.documentType==='supplied'&&area==='pbom'?'관련부품':areaNames[area]}</span>{onArea&&<button disabled={busy} onClick={()=>onArea(area)}>상세 보기</button>}</div>)}</div>
   </section>
   <details className="analysis-detail-card"><summary>상세 보기</summary><div className="analysis-detail-body"><section><h3>AI 분석 설명</h3><p>{analysisSummary(draft)}</p></section>{missing.length>0&&<section><h3>미확인 DATA</h3><ul>{missing.map((item,index)=><li key={`${item.field}:detail:${index}`}><strong>{item.field}</strong><span>{item.reason}</span></li>)}</ul></section>}{draft.documentType!=='supplied'&&<section><h3>생성 DATA 상세</h3>{analysisAreas(draft).map(area=>{const items=areaItems(draft,area);return <details key={area} className="analysis-area-detail"><summary>{draft.documentType==='supplied'&&area==='pbom'?'관련부품':areaNames[area]} · {items.length}건</summary>{items.length?<ul>{items.map(item=><li key={item.id}><strong>{item.title}</strong><span>{item.detail}</span>{item.area==='trr'&&<span>반영 목차: {TRR_SECTIONS.indexOf(item.trrSection!)+1}. {item.trrSection} · 출처: {item.source}</span>}</li>)}</ul>:<p>생성된 DATA가 없습니다.</p>}</details>})}</section>}</div></details>
   ...
   <div className="analysis-history-list">{history.map((row,index)=>{const rowMissing=legacyMissingData(row.draft);return <article key={row.id} className={selected.includes(row.id)?'selected':''}><input type="checkbox" aria-label={`분석 v${row.version} 선택`} disabled={busy||(!selected.includes(row.id)&&selected.length>=5)} checked={selected.includes(row.id)} onChange={e=>{setChat(false);setSelected(ids=>e.target.checked?[...ids,row.id]:ids.filter(id=>id!==row.id))}}/><div className="analysis-history-content"><div className="analysis-history-heading"><div><b>분석 v{row.version}</b>{index===0&&<em>최신</em>}</div><small>{new Date(row.updatedAt*1000).toLocaleString('ko-KR')} · {row.updatedBy}</small></div><div className="analysis-history-counts">{analysisAreas(row.draft).map(area=><span key={area}>{row.draft.documentType==='supplied'&&area==='pbom'?'관련부품':areaNames[area]} <b>{areaItems(row.draft,area).length}</b></span>)}<span className={rowMissing.length?'missing':'ok'}>미확인 <b>{rowMissing.length}</b></span></div><details className="analysis-history-detail"><summary>상세 보기</summary><div><p className="analysis-history-summary">{analysisSummary(row.draft)}</p>{rowMissing.length>0&&<section><h4>미확인 DATA</h4><ul>{rowMissing.map((item,i)=><li key={`${item.field}:${i}`}><strong>{item.field}</strong><span>{item.reason}</span></li>)}</ul></section>}{row.draft.documentType!=='supplied'&&<section><h4>생성 DATA</h4>{analysisAreas(row.draft).map(area=>{const items=areaItems(row.draft,area);return <details key={area}><summary>{row.draft.documentType==='supplied'&&area==='pbom'?'관련부품':areaNames[area]} · {items.length}건</summary>{items.length?<ul>{items.map(item=><li key={item.id}><strong>{item.title}</strong><span>{item.detail}</span></li>)}</ul>:<p>생성된 DATA가 없습니다.</p>}</details>})}</section>}</div></details></div></article>})}</div>
