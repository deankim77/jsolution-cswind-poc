 ...
 return <section className="customer-analysis-results">
  ...
  {tab==='latest'?loading?<p className="analysis-empty">최신 분석 결과를 불러오는 중…</p>:!draft?<p className="analysis-empty">AI 분석을 실행하면 최신 분석 결과를 확인할 수 있습니다.</p>:draft.analysisMode==='text'&&draft.documentType!=='supplied'?<section className="analysis-card"><h2>AI 분석 결과</h2><p className="analysis-history-summary">{analysisSummary(draft)}</p></section>:<div className="analysis-latest">
   ...
   <section className="analysis-card">
    ...
    <div className="analysis-data-counts">{analysisAreas(draft).map(area=><div key={area}><strong>{areaItems(draft,area).length}</strong><span>{areaNames[area]}</span>{onArea&&<button disabled={busy} onClick={()=>onArea(area)}>상세 보기</button>}</div>)}</div>
   </section>
