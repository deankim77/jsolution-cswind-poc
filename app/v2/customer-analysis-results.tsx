...
type Props={url:string;recordId:string;version:number;projectId:string;projectName:string;fileName:string;busy:boolean;canReview:boolean;hasConfirmed:boolean;onBusy:(value:boolean)=>void;onApplied:()=>void;children?:ReactNode};
type ReviewListResponse={reviews:ReviewState[]};
const areaNames:Record<ReviewArea,string>={pbom:'PBOM',trr:'TRR',readiness:'Readiness',work:'작업관리'};
...
export default function CustomerAnalysisResults({url,recordId,version,projectId,projectName,fileName,busy,canReview,onBusy,onApplied}:Props){
 ...
 return <section className="customer-analysis-results">
  ...
  {tab==='latest'?loading?<p className="analysis-empty">최신 분석 결과를 불러오는 중…</p>:!draft?<p className="analysis-empty">AI 분석을 실행하면 최신 분석 결과를 확인할 수 있습니다.</p>:<div className="analysis-latest">
   <section className="analysis-card analysis-type-card">
    <div className="analysis-card-heading"><div><h2>문서 타입 확정</h2><p>AI 분류 결과를 확인하고 맞는 문서 타입을 확정하세요.</p></div><span className={draft.documentTypeConfirmed?'confirmed':'pending'}>{draft.documentTypeConfirmed?'확정됨':'확인 필요'}</span></div>
   ...
   <section className="analysis-card">
    <div className="analysis-card-heading"><div><h2>AI 분석 요약</h2><p>원본에서 가져와야 하지만 확인하지 못한 DATA만 표시합니다.</p></div><span className={missing.length?'pending':'confirmed'}>{missing.length?`미확인 ${missing.length}건`:'미확인 없음'}</span></div>
   ...
   <section className="analysis-card">
    ...
    <div className="analysis-data-counts">{(Object.keys(REVIEW_AREAS) as ReviewArea[]).map(area=><div key={area}><strong>{areaItems(draft,area).length}</strong><span>{areaNames[area]}</span></div>)}</div>
   </section>
   <details className="analysis-detail-card"><summary>상세 보기</summary><div className="analysis-detail-body"><section><h3>AI 분석 설명</h3><p>{draft.summary}</p></section>{missing.length>0&&<section><h3>미확인 DATA</h3><ul>{missing.map((item,index)=><li key={`${item.field}:detail:${index}`}><strong>{item.field}</strong><span>{item.reason}</span></li>)}</ul></section>}<section><h3>생성 DATA 상세</h3>{(Object.keys(REVIEW_AREAS) as ReviewArea[]).map(area=>{const items=areaItems(draft,area);return <details key={area} className="analysis-area-detail"><summary>{areaNames[area]} · {items.length}건</summary>{items.length?<ul>{items.map(item=><li key={item.id}><strong>{item.title}</strong><span>{item.detail}</span></li>)}</ul>:<p>생성된 DATA가 없습니다.</p>}</details>})}</section></div></details>
