...
export function BomCompareWorkspace({initialLeftRootId,initialRightRootId,onClose}:{initialLeftRootId:string;initialRightRootId?:string;onClose:()=>void}){
  ...
  return <>
    <section className="bcw-shell">
      <header className="bcw-head">
        <div className="bcw-toolbar">
          {result&&<div className="bcw-summary"><span>추가 <b>{result.diff.summary.added}</b></span><span>삭제 <b>{result.diff.summary.removed}</b></span><span>수량 변경 <b>{result.diff.summary.quantityChanged}</b></span><span>위치 변경 <b>{result.diff.summary.moved}</b></span></div>}
          <label><input type="checkbox" checked={changesOnly} onChange={event=>setChangesOnly(event.target.checked)}/>변경 항목만 보기</label>
          <button type="button" className="bcw-swap" onClick={swapSides} disabled={comparing||!leftSeq||!rightSeq} title="기존 BOM과 비교 BOM 전환" aria-label="기존 BOM과 비교 BOM 전환"><GitCompareArrows size={18}/></button>
        </div>
        <div className="bcw-head-actions"><button disabled={!result||comparing} onClick={()=>setAiOpen(true)}><Sparkles size={18}/>AI 대화</button><button onClick={onClose}><X size={18}/>닫기</button></div>
      </header>
      {notice&&<p className="bcw-notice">{notice}</p>}
      <main className="bcw-main">
        <div className="bcw-trees">
          <section><header><b title={leftPart?.name}>{leftDisplay}</b>{revisionOption(leftRevisions,leftSeq,setLeftSeq,"기존 BOM Revision")}<span>기존 BOM</span></header>{result?renderTree(result.fromSnapshot,"from"):<div className="bcw-empty">{loading||comparing?"BOM 데이터를 불러오는 중…":sameBom&&leftSeq===rightSeq?"서로 다른 Revision을 선택해 주세요.":"비교할 Revision을 선택해 주세요."}</div>}</section>
          <section><header><b title={rightPart?.name}>{rightDisplay}</b>{revisionOption(rightRevisions,rightSeq,setRightSeq,"비교 BOM Revision")}<span>비교 BOM</span></header>{result?renderTree(result.toSnapshot,"to"):<div className="bcw-empty">{loading||comparing?"BOM 데이터를 불러오는 중…":sameBom&&leftSeq===rightSeq?"서로 다른 Revision을 선택해 주세요.":"비교할 Revision을 선택해 주세요."}</div>}</section>
        </div>
      </main>
    </section>
