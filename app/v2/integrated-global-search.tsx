...
export function IntegratedGlobalSearch({query,setQuery,onClose,onOpen,onAi,embedded=true}:{query:string;setQuery:(value:string)=>void;onClose:()=>void;onOpen:(result:IntegratedSearchResult)=>void;onAi:(items:ContextItem[],prompt:string)=>void;embedded?:boolean}){
  ...
  useEffect(()=>{
    setError("");
    if(!query.trim()){setResults([]);setIntent("일반 검색");setLoading(false);return}
    setLoading(true);
    const controller=new AbortController(),timer=window.setTimeout(()=>{
      fetch(`/api/search?q=${encodeURIComponent(query.trim())}&scope=${scope}&intent=1`,{cache:"no-store",signal:controller.signal})
        .then(async response=>{
          const data=await readApiJson(response);
          if(!response.ok)throw new Error(data.error||"검색하지 못했습니다.");
          if(controller.signal.aborted)return;
          setResults(data.results??[]);setIntent(data.intentLabel||"일반 검색");
        })
        .catch(reason=>{if(!controller.signal.aborted){setResults([]);setError(reason instanceof Error?reason.message:"검색하지 못했습니다.")}})
