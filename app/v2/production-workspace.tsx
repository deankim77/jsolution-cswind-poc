...
export default function ProductionWorkspace({project,tab,onCloseAi,onOpenFilter,onOpenBomEditor,panelHidden=false}:{onOpenBomEditor:(rootId:string)=>void;onOpenFilter:(config:WorkspaceFilterConfig)=>void;project:V2Project;tab:ProductionTab;panelHidden?:boolean;onCloseAi:()=>void}){
 ...
 const analyze=()=>run(async()=>{const result=await post({recordId:selected,recordIds:[...new Set([selected,...checked])],message:'선택 문서를 근거로 문서 종류를 분류하고 PBOM·TRR·Process Readiness·공정별 작업관리에 반영할 초안을 작성해 주세요. 원본에 없는 내용은 만들지 말고 불확실한 점을 표시하세요.'});accept(result)});
 ...
 })}{!rows.length&&<tr><td colSpan={10}>{!data&&!error?'자료를 불러오는 중…':'표시할 문서가 없습니다.'}</td></tr>}</tbody></table></div> :tab==='pbom'?<>{pbom?.root?<PbomTable toolbarContainer={pbomToolbar} renderSource={renderSource} rows={pbom.rows.filter(r=>!query||`${JSON.stringify(r)} ${(r.sourceRecordIds??[r.recordId]).map(id=>{const source=data?.records.find(d=>d.id===id);return source?customerReceiptId(source):id}).join(' ')}`.toLowerCase().includes(query.toLowerCase()))} root={pbom.root} onOpenEditor={()=>onOpenBomEditor(pbom.root!.id)}/>:!pbomError&&<p className="production-help">{pbom?'기존 프로젝트의 TOP 품목을 생성하세요.':'BOM을 불러오는 중…'} {pbom&&data?.canReview&&<button disabled={busy} onClick={()=>void run(async()=>{const r=await fetch(`/api/projects/${project.id}/pbom`,{method:'POST'});const d=await r.json();if(!r.ok)throw Error(d.error);refresh()})}>TOP 품목 생성</button>}</p>}</>:<><p className="production-help">2번에서 담당자가 확정한 DATA입니다. 원본·결정 버전을 함께 보관합니다.</p><ReviewRequirementsTable renderSource={renderSource} rows={confirmedRows.filter(c=>`${JSON.stringify(c.item)} ${data?.records.find(r=>r.id===c.item.recordId)?customerReceiptId(data.records.find(r=>r.id===c.item.recordId)!):''}`.toLowerCase().includes(query.toLowerCase())).map(c=>({item:c.item,decision:`확정 v${c.version} · ${uploadTime(c.confirmedAt)}`}))}/></>}
 </section>
