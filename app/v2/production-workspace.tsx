...
export const productionTabs=[['customer','고객 Data'],['review','AI Data Review'],['pbom','BOM'],['extract','AI 추출사항'],['ttr','TRR']] as const;
...
export default function ProductionWorkspace({project,tab,onCloseAi,onOpenFilter,onOpenBomEditor,panelHidden=false}:{onOpenBomEditor:(rootId:string)=>void;onOpenFilter:(config:WorkspaceFilterConfig)=>void;project:V2Project;tab:ProductionTab;panelHidden?:boolean;onCloseAi:()=>void}){
 ...
 {Object.entries(REVIEW_AREAS).map(([area,label])=>{if(!reviewsLoaded||(area==='trr'&&!trr))return <td key={area}>불러오는 중…</td>;const baseCount=reviewAreaCounts(review,confirmed,area as ReviewArea);const applied=trr?.appliedSources?.find(s=>s.id===r.id);const count=area==='trr'?{...baseCount,approved:applied?.factCount??0}:baseCount;
 const resultLabel=area==='trr'?'반영':'확정';
 const description=`최신 추출 ${count.analyzed===null?'없음':`${count.analyzed}건`} · 기존 ${resultLabel} ${count.approved}건${count.conditional?` (조건부 ${count.conditional}건 포함)`:''}. 추출과 반영은 서로 다른 버전일 수 있습니다.`;
 return <td className="review-count-column" key={area}><button type="button" disabled={busy||dirty} onClick={e=>{e.stopPropagation();choose(r.id,area)}} title={description} aria-label={`${customerReceiptId(r)} ${label.replace(/^\d+\. /,'')}: ${description} 결과 보기`}><span className={`review-result-badge ${count.conditional?'is-conditional':count.approved>0?'has-result':'is-empty'}`}>{resultLabel} <strong>{count.approved}</strong>건{count.conditional?' · 조건부':''}</span><span className="review-result-extracted">최신 추출 {count.analyzed===null?'—':`${count.analyzed}건`}</span></button></td>})}
 </tr>;
 ...
 </>:!analyzing&&<p>AI 분석을 실행한 뒤 추출 결과를 검토하세요. TRR은 검토 후 반영 버튼으로 Word에 저장합니다.</p>}
