"use client";
import CustomerSuppliedWorkspace from './customer-supplied-workspace';
import CustomerSuppliedReview from './customer-supplied-review';
...
import TrrReviewDetails from './trr-review-details';
...
export default function ProductionWorkspace({project,tab,onCloseAi,onOpenFilter,onOpenBomEditor,panelHidden=false}:{onOpenBomEditor:(rootId:string)=>void;onOpenFilter:(config:WorkspaceFilterConfig)=>void;project:V2Project;tab:ProductionTab;panelHidden?:boolean;onCloseAi:()=>void}){
 ...
 const [extractToolbar,setExtractToolbar]=useState<HTMLDivElement|null>(null);
 const [pbom,setPbom]=useState<ProjectPbom|null>(null),[pbomError,setPbomError]=useState('');
 const [trr,setTrr]=useState<TrrList|null>(null);
 const [trrActions,setTrrActions]=useState<HTMLDivElement|null>(null);
 ...
 const rows=(data?.records??[]).filter(r=>{const d=reviews.find(v=>v.recordId===r.id)?.draft;return (r.sourcePurpose!=='template'&&r.sourcePurpose!=='example')&&(!type||(d?.documentType??'unclassified')===type)&&(!status||(status==='analyzed'?Boolean(d):!d))&&`${customerReceiptId(r)} ${r.fileName} ${d?.drawingNumber??''} ${d?.summary??''}`.toLowerCase().includes(query.toLowerCase())});
 ...
 const reflectTrr=(action:'confirm'|'cancel',itemIds?:string[])=>run(async()=>{setNotice('');const response=await fetch(`/api/projects/${project.id}/trr`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({recordId:selected,version:current?.version,wordVersion:trr?.versions[0]?.version??0,action,itemIds})}),result=await readApiJson(response);if(!response.ok)throw Error(result.error);setNotice(`TRR ${trrVersionLabel(result.version)} ${action==='cancel'?'확정 취소를 반영했습니다.':'선택 항목을 확정했습니다.'}`);refresh()});
 const post=async(body:unknown)=>{const r=await fetch(`${url}/review`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body)}),d=await readApiJson(r);if(!r.ok)throw Error(d.error);return d;};
 ...
 const choose=(id:string,area='summary')=>{if(busy||dirty||!reviewsLoaded)return;onCloseAi();setSelected(id);setReviewTab(area);setPanelWide(false);setNotice('');setError('');};
 ...
 useEffect(()=>{if(tab!=='review')return;const c=new AbortController();fetch(`/api/projects/${project.id}/trr`,{signal:c.signal,cache:'no-store'}).then(async r=>{const d=await readApiJson(r);if(!r.ok)throw Error(d.error);setTrr(d)}).catch(e=>{if(!c.signal.aborted)setError(e.message)});return()=>c.abort()},[project.id,tab,reload]);
 ...
 if(tab==='supplied')return <CustomerSuppliedWorkspace key={project.id} projectId={project.id}/>;
 ...
 return <><div className="wv2-toolbar"><label><Search size={18}/><input value={query} disabled={busy||dirty} onChange={e=>setQuery(e.target.value)} placeholder={tab==='review'?'자료 ID·파일명·분석 내용 검색':'확정 DATA 검색'}/></label>{tab==='review'&&<button disabled={busy||dirty} onClick={()=>{setSelected('');setFilterDraft({type,status});setFilterOpen(true)}}><Filter size={18}/>상세 필터</button>}<button disabled={busy||dirty} onClick={refresh}><RefreshCw size={18}/>새로고침</button>{tab==='pbom'&&Boolean(pbom?.pendingDocuments)&&<button disabled={busy||!data?.canReview} onClick={()=>void run(async()=>{const r=await fetch(`/api/projects/${project.id}/pbom`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'reconcile'})});const result=await readApiJson(r);if(!r.ok)throw Error(result.error);refresh();const failed=result.results.filter((item:{ok:boolean})=>!item.ok);if(failed.length)throw Error(failed.map((item:{recordId:string;error:string})=>`${(()=>{const source=data?.records.find(record=>record.id===item.recordId);return source?`${customerReceiptId(source)} · ${source.fileName}`:item.recordId})()}: ${item.error}`).join(' / '));})}><RefreshCw size={18}/>미반영 {pbom?.pendingDocuments}개 문서 연결</button>}{tab==='pbom'&&pbom?.root&&<button onClick={()=>onOpenBomEditor(pbom.root!.id)}>BOM 편집기 · 구매품/공용품 추가</button>}{tab==='pbom'&&<div className="pbom-toolbar-slot" ref={setPbomToolbar}/>} {tab==='extract'&&<div className="ai-extract-toolbar-slot" ref={setExtractToolbar}/>} {(tab==='pbom'||tab==='extract')&&<button type="button" className="production-edit-all" onClick={()=>{const popup=window.open(`/production-editor?projectId=${encodeURIComponent(project.id)}&area=${tab}&name=${encodeURIComponent(project.name)}`,`production-edit-${project.id}-${tab}`,`popup=yes,width=${window.screen.availWidth},height=${window.screen.availHeight},left=0,top=0,resizable=yes,scrollbars=yes`);if(popup)popup.focus();else setError('팝업 차단을 해제하고 다시 열어주세요.');}}><PencilLine size={18}/>전체수정</button>}<span className="cswind-data-count">{tab==='review'?`${rows.length}개 문서`:tab==='pbom'?`${pbom?.rows.length??0}개 BOM 행`:`${confirmedRows.length}개 확정 항목`}</span></div>
 ...
 {tab==='review'?<><p className="production-help">① 문서 선택 · AI 분석 → ② 추출 결과 검토·보완 → ③ TRR 반영 · Word 새 버전 저장</p><div className="cswind-data-table"><table className="production-table customer-document-list"><thead><tr><th>비교</th><th>미리보기</th><th>자료 ID</th><th>원본 파일명</th><th>문서 종류</th><th>분석 상태</th>{Object.entries(REVIEW_AREAS).map(([key,label])=><th className="review-count-column" key={key} title={label.replace(/^\d+\. /,'')}>{key==='pbom'?'BOM':key==='trr'?'TRR':'AI 추출'} (추출/반영)</th>)}</tr></thead><tbody>{rows.map(r=>{
 const review=reviews.find(v=>v.recordId===r.id),job=analysis.jobs.find(j=>j.recordId===r.id),appliedSource=trr?.appliedSources?.find(s=>s.id===r.id);
 const rowStatus=job&&job.status!=='completed'?analysisJobLabel(job):r.sourcePurpose==='ttr'?(review?(appliedSource?.reviewVersion===review.version?'TRR 반영 완료':'추출 완료 · 검토 대기'):'추출 대기'):(review?'AI 분석 완료':'미분석');
 ...
 <td>{reviewsLoaded?REVIEW_TYPES[review?.draft.documentType??'unclassified']:'불러오는 중…'}</td><td>{reviewsLoaded?rowStatus:'불러오는 중…'}</td>
 {Object.entries(REVIEW_AREAS).map(([area,label])=>{if(!reviewsLoaded||(area==='trr'&&!trr))return <td key={area}>불러오는 중…</td>;const baseCount=reviewAreaCounts(review,confirmed,area as ReviewArea);const applied=trr?.appliedSources?.find(s=>s.id===r.id);const count=area==='trr'?{...baseCount,approved:applied?.factCount??0}:baseCount;return <td className="review-count-column" key={area} onClick={e=>{e.stopPropagation();choose(r.id,area)}}><button type="button" disabled={busy||dirty} title={`최신 분석 / 승인 완료 (조건부 승인 포함)${count.conditional?` · 조건부 승인 ${count.conditional}건 (확정 포함)`:''}`} aria-label={`${customerReceiptId(r)} ${label.replace(/^\d+\. /,'')} 분석 ${count.analyzed??'없음'}건, 확정 ${count.approved}건, 조건부 승인 ${count.conditional}건. 결과 보기`}><span>{count.analyzed??'—'} / <strong>{count.approved}</strong></span></button></td>})}
 </tr>;
 })}{!rows.length&&<tr><td colSpan={9}>{!data&&!error?'자료를 불러오는 중…':'표시할 문서가 없습니다.'}</td></tr>}</tbody></table></div></> :tab==='pbom'?<>{pbom&&(pbom.root||pbom.rows.length)?<PbomTable toolbarContainer={pbomToolbar} renderSource={renderSource} rows={pbom.rows.filter(r=>!query||`${JSON.stringify(r)} ${(r.sourceRecordIds??[r.recordId]).map(id=>{const source=data?.records.find(d=>d.id===id);return source?customerReceiptId(source):id}).join(' ')}`.toLowerCase().includes(query.toLowerCase()))} root={pbom.root} onOpenEditor={pbom.root?()=>onOpenBomEditor(pbom.root!.id):undefined}/>:!pbomError&&<p className="production-help">{pbom?'기존 프로젝트의 TOP 품목을 생성하세요.':'BOM을 불러오는 중…'} {pbom&&data?.canReview&&<button disabled={busy} onClick={()=>void run(async()=>{const r=await fetch(`/api/projects/${project.id}/pbom`,{method:'POST'});const d=await readApiJson(r);if(!r.ok)throw Error(d.error);refresh()})}>TOP 품목 생성</button>}</p>}</>:<><ReviewRequirementsTable toolbarContainer={extractToolbar} rows={confirmedRows.filter(c=>`${JSON.stringify(c.item)}`.toLowerCase().includes(query.toLowerCase())).map(c=>({item:c.item,decision:`확정 v${c.version} · ${uploadTime(c.confirmedAt)}`}))}/></>}
 </section>
 ...
 <nav>{(record.sourcePurpose==='supplied'?[['summary','AI 분석 결과'],['supplied','사급품'],['pbom','BOM'],['ai','AI 대화']]:[['summary','AI 분석 결과'],...Object.entries(REVIEW_AREAS).map(([key,label])=>[key,label.replace(/^\d+\. /,'')]),['ai','AI 대화']]).map(([key,label])=><button key={key} className={reviewTab===key?'active':''} disabled={busy||(dirty&&key==='ai')} onClick={()=>selectTab(key)}>{label}{record.sourcePurpose==='supplied'&&['supplied','pbom'].includes(key)?` (${draft?.suppliedItems?.length??0})`:draft&&key in REVIEW_AREAS?` (${draft.items.filter(i=>i.area===key).length})`:''}</button>)}</nav>
 <section className="wv2-panel-body review-panel-body">
 <div className="review-draft">
 <div className="review-decision review-confirm-actions"><button onClick={()=>popup(record.id)}><ExternalLink size={18}/>원본 팝업</button><button disabled={mutationBusy||dirty||!data?.canReview} onClick={()=>{if(current&&!window.confirm('새 분석 초안을 생성할까요? 현재 보완 내용과 기존 Word는 이력에 보존됩니다.'))return;void analyze()}}><RefreshCw size={18}/>{analyzing?'분석 중…':current?'재추출':'AI 분석'}</button>{draft&&reviewTab!=='ai'&&<div className="review-save-actions">{dirty&&<><button disabled={busy} onClick={()=>void run(async()=>{accept(await post({action:'save',recordId:selected,version:current?.version,draft}));setDirty(false)})}><Save size={18}/>초안 저장</button><button disabled={busy} onClick={()=>{setDraft(current?.draft??null);setDirty(false)}}>수정 취소</button></>}{record.sourcePurpose!=='supplied'&&reviewTab!=='summary'&&reviewTab!=='trr'&&<ReviewDecisionActions key={`${selected}:${reviewTab}`}
 ...
 />}{reviewTab==='trr'&&<div className="trr-action-slot" ref={setTrrActions}/>} </div>}</div>
 {error&&<p role="alert" className="wv2-form-error">{error}</p>}{notice&&<p role="status">{notice}</p>}{busy&&<p role="status">{reviewTab==='trr'?'TRR 검토 내용 저장·반영 중입니다…':'처리 중입니다…'}</p>}
 ...
 {record.sourcePurpose==='supplied'?<>{reviewTab==='summary'&&<CustomerAnalysisResults key={selected} url={`${url}/review`} recordId={selected} version={current?.version??0} projectId={project.id} projectName={project.name} fileName={record.fileName} busy={mutationBusy||dirty} canReview={Boolean(data?.canReview)} hasConfirmed={false} onBusy={setBusy} onApplied={refresh}/>}<CustomerSuppliedReview key={`${selected}:${reviewTab}`} projectId={project.id} recordId={selected} version={current?.version??0} facts={draft.suppliedItems??[]} area={reviewTab==='pbom'?'bom':reviewTab==='supplied'?'supplied':'summary'} disabled={mutationBusy||!data?.canReview} onApplied={refresh}/></>:reviewTab==='summary'?<CustomerAnalysisResults key={selected} url={`${url}/review`} recordId={selected} version={current?.version??0} projectId={project.id} projectName={project.name} fileName={record.fileName} busy={mutationBusy||dirty} canReview={Boolean(data?.canReview)} hasConfirmed={confirmed.some(c=>c.recordId===selected&&c.item.area==='pbom')} onBusy={setBusy} onApplied={refresh} onArea={selectTab}/>:reviewTab==='trr'?<TrrReviewDetails key={selected} actionContainer={trrActions} draft={draft} version={current?.version??0} confirmedCount={trr?.appliedSources?.find(s=>s.id===selected)?.factCount??0} documentName={record.fileName} disabled={mutationBusy||!data?.canReview} dirty={dirty} onCancel={()=>void reflectTrr('cancel')} onApply={ids=>void reflectTrr('confirm',ids)}/>:reviewTab==='pbom'?<>{bomError&&<p role="alert">{bomError}</p>}{areaItems.some(i=>!i.bom)&&<p>이전 분석은 텍스트 형식입니다. 재검증하면 BOM 구조와 필수 컬럼을 추출합니다.</p>}<PbomTable renderSource={renderSource} variant="review" rows={bomRows} editable={!mutationBusy&&Boolean(data?.canReview)} onEdit={async(id,bom)=>{setBusy(true);try{const next={...draft,items:draft.items.map(i=>i.id===id?{...i,bom}:i)};const result=await post({action:'save',recordId:selected,version:current?.version,draft:next});accept(result);setDraft(result.review?.draft??next);setDirty(false);setItemIds([]);setNotice('부품 정보가 저장되었습니다.')}finally{setBusy(false)}}}/></>:<ReviewRequirementsTable rows={areaItems.map(item=>({item}))} selected={itemIds} disabled={mutationBusy||!data?.canReview} onSelect={ids=>setItemIds(ids)} onEdit={async item=>{setBusy(true);try{const next={...draft,items:draft.items.map(currentItem=>currentItem.id===item.id?item:currentItem)};const result=await post({action:'save',recordId:selected,version:current?.version,draft:next});accept(result);setDraft(result.review?.draft??next);setDirty(false);setItemIds([]);setNotice('AI 추출사항을 저장했습니다.')}finally{setBusy(false)}}}/>}
 </>:!analyzing&&<p>AI 분석을 실행한 뒤 추출 결과를 검토하세요.</p>}
