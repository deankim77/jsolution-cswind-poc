...
export default function ProductionWorkspace({project,tab,onCloseAi,onOpenFilter,onOpenBomEditor,panelHidden=false}:{onOpenBomEditor:(rootId:string)=>void;onOpenFilter:(config:WorkspaceFilterConfig)=>void;project:V2Project;tab:ProductionTab;panelHidden?:boolean;onCloseAi:()=>void}){
 ...
 return <td className="review-count-column" key={area}><button type="button" disabled={busy||dirty} onClick={e=>{e.stopPropagation();choose(r.id,area)}} title={`${area==='trr'?'추출 / 반영':'분석 / 확정'}${count.conditional?` · 조건부 승인 ${count.conditional}건 (확정 포함)`:''}`} aria-label={`${customerReceiptId(r)} ${label.replace(/^\d+\. /,'')} ${area==='trr'?'추출':'분석'} ${count.analyzed??'없음'}건, ${area==='trr'?'반영':'확정'} ${count.approved}건. 결과 보기`}><span>{count.analyzed??'—'} / <strong className="review-applied-count">{count.approved}</strong></span></button></td>})}
 </tr>;
