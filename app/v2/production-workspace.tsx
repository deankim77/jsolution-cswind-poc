...
import ProductionBulkDialog from './production-bulk-dialog';
...
export default function ProductionWorkspace({project,tab,onCloseAi,onOpenFilter,onOpenBomEditor,panelHidden=false}:{onOpenBomEditor:(rootId:string)=>void;onOpenFilter:(config:WorkspaceFilterConfig)=>void;project:V2Project;tab:ProductionTab;panelHidden?:boolean;onCloseAi:()=>void}){
 ...
 const [extractToolbar,setExtractToolbar]=useState<HTMLDivElement|null>(null),[bulkArea,setBulkArea]=useState<ReviewArea|null>(null);
 ...
 {bulkArea&&<ProductionBulkDialog key={`${project.id}:${bulkArea}`} projectId={project.id} projectName={project.name} sources={Object.fromEntries((data?.records??[]).map(record=>[record.id,customerReceiptId(record)]))} area={bulkArea} onClose={()=>setBulkArea(null)} onSaved={refresh}/>}
 </>;
}
