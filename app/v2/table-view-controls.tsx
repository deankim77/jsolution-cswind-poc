...
export function ColumnVisibilityMenu<K extends string>({options,visible,onChange,onReset,open,onOpenChange}:{options:ReadonlyArray<{key:K;label:string;locked?:boolean}>;visible:ReadonlySet<K>;onChange:(next:Set<K>)=>void;onReset:()=>void;open:boolean;onOpenChange:(open:boolean)=>void}){
 ...
 return <div ref={menuRef} className="wv2-toolbar-menu table-column-menu" onKeyDown={event=>{if(event.key==="Escape"){onOpenChange(false);event.stopPropagation();}}} onBlur={event=>{if(event.relatedTarget&&!event.currentTarget.contains(event.relatedTarget))onOpenChange(false);}}>
  ...
  {open&&<div id={id} className="table-column-popover" role="group" aria-label="표시할 열"><b>표시할 열</b>
   {options.map(({key,label,locked})=><label key={key}><input type="checkbox" checked={Boolean(locked)||visible.has(key)} disabled={locked} onChange={event=>{const next=new Set(visible);if(event.currentTarget.checked)next.add(key);else next.delete(key);onChange(next);}}/><span>{label}</span></label>)}
