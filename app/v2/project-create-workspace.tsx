...
import {ProductionSetup} from "./production-setup";
import type {V2Project} from "./project-workspaces";

type TemplateWbsItem={id:string;name:string;level?:number};
type TemplateItem={id:string;code:string;name:string;status:string;versionId:string;version:string;definition:{projectTypeCode?:string;description?:string;roles?:string[];wbs?:TemplateWbsItem[]}};
...
export function ProjectCreateWorkspace({onCancel,onCreated}:{onCancel:()=>void;onCreated:(project:V2Project)=>void}){
  const [entryType,setEntryType]=useState("RD");
  ...
  const [projectTypes,setProjectTypes]=useState<Array<{id:string;code:string;name:string;status:string}>>([]);
  ...
  useEffect(()=>{
    Promise.all([
      ...
      setTemplates(active);
      setSelectedTemplate(active.find((item:TemplateItem)=>(item.definition.projectTypeCode||"RD")===entryType)?.versionId??"");
      setCopyProjects(sources.filter((item:V2Project)=>item.projectTypeCode!=="PRODUCTION"));
      setSelectedCopy(sources.find((item:V2Project)=>item.projectTypeCode!=="PRODUCTION")?.id??"");
      setPartners(activePartners);
      setProjectTypes(activeTypes);
      setOrganizations(activeOrganizations);
      setForm(value=>({...value,code:codeData.code??value.code,partnerId:activePartners[0]?.id||"",projectTypeId:activeTypes.find((item:{id:string;code:string})=>entryType==="PRODUCTION"?item.code==="PRODUCTION":item.code!=="PRODUCTION")?.id||"",customerName:activePartners[0]?.name||value.customerName}));
    }).catch(()=>setError("프로젝트 생성 정보를 불러오지 못했습니다."));
  },[reload]);

  useEffect(()=>{
    setMode("template");
    setSelectedTemplate(templates.find(item=>(item.definition.projectTypeCode||"RD")===entryType)?.versionId||"");
    setForm(value=>({...value,projectTypeId:projectTypes.find(item=>entryType==="PRODUCTION"?item.code==="PRODUCTION":item.code!=="PRODUCTION")?.id||""}));
    const controller=new AbortController();
    fetch(`/api/projects?action=next-code&projectTypeCode=${entryType}`,{cache:"no-store",signal:controller.signal}).then(async response=>{const result=await response.json();if(!response.ok)throw new Error(result.error);return result;}).then(result=>setForm(value=>({...value,code:result.code}))).catch(reason=>{if(!controller.signal.aborted)setError(reason.message)});
    return ()=>controller.abort();
  },[entryType,templates,projectTypes]);
  const template=templates.find(item=>item.versionId===selectedTemplate);
  const submit=async()=>{
    setError("");
    if(entryType==="PRODUCTION"&&mode!=="template"){setError("생산 프로젝트는 생산용 템플릿으로 생성하세요.");return}
    ...
    try{
      ...
      onCreated({id:result.id,code:result.code||form.code,name:form.name,customerName:form.customerName,partnerId:form.partnerId,partnerName:partner?.name,projectTypeId:form.projectTypeId,projectTypeName:projectType?.name,projectTypeCode:projectType?.code,description:form.description,referenceCode:form.referenceCode,visibility:form.visibility,startDate:form.startDate,endDate:form.endDate,status:"preparing",templateName:mode==="copy"?`복사 · ${copied?.name||"기존 프로젝트"}`:template?.name,templateVersion:mode==="copy"?undefined:template?.version});
