...
export function createTrrService(repo=createTrrRepository(),raw=createCustomerDataRepository(),storage=getStorageAdapter(),reviews=createCustomerReviewRepository()){
 return {
  ...
  async generate(s:CustomerDataScope,input:{recordId:string;version:number}){
   ...
   try{
    ...
    const source:TrrSource={id:record.id,rawDataId:record.rawDataId,fileName:record.fileName,checksum:record.checksum,revision:record.revision,reviewVersion:current.version,kind:items.every(i=>i.trrKind==='historical')?'historical':'current',summary:draft.summary,facts:items.map(i=>({id:i.id,kind:i.trrKind,section:i.trrSection!,title:i.title,detail:i.detail,reference:i.source}))};
    // Replace this document's reviewed snapshot; preserve other sources and old Word versions.
    const sources=[...(latest?.document.sources??[]).filter(src=>src.rawDataId!==source.rawDataId),source].sort((a,b)=>a.rawDataId.localeCompare(b.rawDataId));
    ...
    const summary=record.fileName+' · 분석 v'+current.version+' 반영 · '+trrChangeSummary(latest?.document.sources??[],sources),id=randomUUID();
