...
type Block={text:string;style?:'Title'|'Subtitle'|'Heading1'|'Heading2';page?:boolean};
export function trrBlocks(doc:TrrDocument):Block[]{
...
export function createTrrDocx(doc:TrrDocument){
 const paragraphs=trrBlocks(doc).map((b,index,blocks)=>`<w:p><w:pPr>${b.style?`<w:pStyle w:val="${b.style}"/>`:''}${b.page?'<w:pageBreakBefore/>':''}${blocks[index+1]?.text.startsWith('근거:')?'<w:keepNext/>':''}</w:pPr><w:r><w:t xml:space="preserve">${escapeXml(b.text)}</w:t></w:r></w:p>`).join('');
 const styles=`<w:styles xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:docDefaults><w:rPrDefault><w:rPr><w:rFonts w:ascii="Arial" w:hAnsi="Arial" w:eastAsia="Malgun Gothic"/><w:sz w:val="22"/></w:rPr></w:rPrDefault><w:pPrDefault><w:pPr><w:spacing w:after="120" w:line="300" w:lineRule="auto"/></w:pPr></w:pPrDefault></w:docDefaults><w:style w:type="paragraph" w:default="1" w:styleId="Normal"><w:name w:val="Normal"/></w:style>${[['Title',40],['Subtitle',26],['Heading1',30],['Heading2',24]].map(([name,size])=>`<w:style w:type="paragraph" w:styleId="${name}"><w:name w:val="${name}"/><w:basedOn w:val="Normal"/><w:pPr><w:keepNext/><w:spacing w:before="240" w:after="160"/>${String(name).startsWith('Heading')?`<w:outlineLvl w:val="${name==='Heading1'?0:1}"/>`:''}</w:pPr><w:rPr><w:b/><w:color w:val="000000"/><w:sz w:val="${size}"/></w:rPr></w:style>`).join('')}</w:styles>`;
