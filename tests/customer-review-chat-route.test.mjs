...
function route({customer=false,failFile=false}={}){
 const calls=[],writes=[];
 const statement=sql=>({bind(...args){writes.push({sql,args});return this},async first(){return {id:'conversation'}},async all(){return {results:[{role:'user',content:'앞선 질문'}]}}});
 ...
 return {calls,writes,send:()=>exports.POST(new Request('http://localhost/api/ai/chat',{method:'POST',body:JSON.stringify({message:'도면 설명',conversationId:'conversation',contextItems:customer?[{id:'raw',kind:'고객 원본',title:'drawing.png'}]:[],...(customer?{customerReview:{projectId:'p',recordId:'raw'}}:{})})}))};
}
