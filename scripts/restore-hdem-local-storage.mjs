...
const docsRoot=resolve(process.env.HDEM_DOCS_DIR||".demo-generated/hdem-equipment-g6");
const storageRoot=resolve(process.env.STORAGE_ROOT||"./storage");
const manifestPath=resolve(docsRoot,"manifest.json");
let manifest;
try{manifest=JSON.parse(await readFile(manifestPath,"utf8"))}catch{throw new Error(`HDEM manifest를 찾을 수 없습니다: ${manifestPath}\n먼저 npm.cmd run data:generate:hdem-docs 를 실행해 주세요.`)}

const projectCode=`${hdemDemo.projectCode}-DEMO`;
const client=new pg.Client({connectionString:process.env.DATABASE_URL});
await client.connect();
let restored=0,alreadyPresent=0,missingVersion=0,missingSource=0,sizeMismatch=0;
try{
  ...
  for(const item of manifest.documents||[]){
    const versionResult=await client.query(`
      ...
      LIMIT 1`,[project.id,item.fileName,item.name,item.task]);
    const version=versionResult.rows[0];
    if(!version){console.warn(`SKIP DB version 없음: ${item.task} / ${item.fileName}`);missingVersion++;continue;}
    ...
    const metadata={contentType:version.contentType||item.mime||"application/octet-stream",customMetadata:{restoredFrom:"HDEM_DEMO_GENERATOR",versionId:version.id,originalName:version.fileName}};
    ...
    restored++;
    console.log(`RESTORED ${item.task} -> ${version.fileKey}`);
  }

  console.log("\nHDEM Local Storage 복원 완료");
